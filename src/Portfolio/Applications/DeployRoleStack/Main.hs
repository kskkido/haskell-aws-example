    ...
    template <- pure
      ( ( Stratosphere.template $ Stratosphere.Resources
            ...
            , ( ( Stratosphere.iamManagedPolicy
                    ( Maybe.fromMaybe mempty $ do
                        let cfnArn = "arn:aws:cloudformation:${AWS::Region}:${AWS::AccountId}:stack/" <> config.serviceName <> "-*"
                        Aeson.toObject
                          [Aeson.QQ.aesonQQ|
                            {
                              "Version": "2012-10-17",
            ...
            , ( ( Stratosphere.iamManagedPolicy
                    ( Maybe.fromMaybe mempty $ do
                        let lambdaArn = "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:" <> config.serviceName <> "-*"
