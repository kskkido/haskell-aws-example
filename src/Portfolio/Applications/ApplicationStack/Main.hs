module Portfolio.Applications.ApplicationStack.Main
  ( main
  ) where
...
import qualified Portfolio.Applications.ApplicationStack.Data.Stack.Main as Stack
import qualified Portfolio.Applications.ApplicationStack.Data.LambdaSource.Main as LambdaSource
    ...
    template <- pure
      ( ( Stratosphere.template $ Stratosphere.Resources
            [ ( ( Stratosphere.lambdaFunction
                    ( ( Stratosphere.lambdaFunctionCode ) &
                      ...
                      ( Stratosphere.lfcS3Key ?~
                          ( Stratosphere.Literal $ Text.pack config.lambdaPushFilePath )
                      )
                    )
            ...
            , ( ( Stratosphere.lambdaVersion
                ...
                ( Stratosphere.lvDescription ?~
                  ( Stratosphere.Literal $ Text.pack config.lambdaVersion )
                ) &
        ...
        ( Stratosphere.templateDescription ?~ "ApplicationStack"
        )
      )
    LambdaSource.upload
