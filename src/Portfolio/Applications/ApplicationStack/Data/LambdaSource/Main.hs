module Portfolio.Applications.ApplicationStack.Data.LambdaSource.Main
  ( upload
  ) where
...
upload = do
  config <- Reader.ask
  source <- liftIO do
    absoluteFilePath <- Lib.FilePath.toAbsolutePath config.lambdaPullFilePath
        ...
        putSourceObject =
          ( ( Amazonka.Data.Body.Hashed $ Amazonka.Data.Body.toHashed source ) &
            ( Amazonka.S3.PutObject.newPutObject
              ( Amazonka.S3.BucketName $ Text.pack config.lambdaBucketName )
              ( Amazonka.S3.ObjectKey $ Text.pack config.lambdaPushFilePath )
            )
          )
