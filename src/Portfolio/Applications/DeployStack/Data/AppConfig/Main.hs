module Portfolio.Applications.DeployStack.Data.AppConfig.Main
  ( AppConfig(..)
  , fromSystem
  , toEnvJson
  ) where
...
toEnvJson :: AppConfig -> Aeson.Object
toEnvJson config = Aeson.KeyMap.fromList $
  second Aeson.toJSON <$>
  [ ("STAGE", config.stage)
